...
(
  ...
  if grep -Eqi '^ID=(.*ubuntu.*|.*debian.*)' /etc/os-release; then
    sudo apt-get -qq install -y wget git tmux ripgrep fzf vim neovim pup jq bc bat
    ...
    sudo yum install -y -q wget git tmux ripgrep fzf vim neovim pup jq bc bat
    # curl https://sh.rustup.rs -sSf -o rust-install.sh && bash rust-install.sh -y -q && rm rust-install.sh
    # cargo install git-delta
    ...
    brew install wget git tmux ripgrep fzf vim neovim pup jq bc bat git-delta
  fi
)
